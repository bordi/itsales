...
function insertHndtst_Code(){
    ...
    var tst_random_single_tst = jQuery('#tst_random_single_tst').is(':checked');
	...
	if(tst_random_single_tst){
		output += 'single_random="yes" ';
	}else if(id){
	   output += 'id="' + id + '" ';        
	}
...
jQuery(document).ready(function($) { //Begin jQuery(document)
  ...
  $('#tst_random_single_tst').change(function(){
	  if($(this).is(':checked')){
		$('#tst_single_wrap'  ).hide();		  
	  }else{
		$('#tst_single_wrap'  ).show();		  
	  }
  });

  // Toggle Advanced Setttings on click (expand/accordian)
  $("#hndtst_tst_generator_advanced_section").hide();
