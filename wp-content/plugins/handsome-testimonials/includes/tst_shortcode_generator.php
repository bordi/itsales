...
global $pagenow;
  ...
  if( !in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php', 'widgets.php', 'edit.php' ) ) )
    return;
...
function hndtst_shortcode_btn ($context) {
  ...
  global $pagenow;
  //Test to make sure we're on an add/edit post/page/post-type/edit screen
...
function hndtst_popup_content($htst_design_page=false) {
  global $pagenow;
  //Test to make sure we're on an add/edit post/page/post-type screen
  if( !in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php', 'edit.php' ) ) )
    return;
?>
    <!--Popup Form HTML -->
    <div id="hndtst-dialog">
        ...
        <div class="hndtst-dialog_wrapper">
                ...
                <form action="/" method="get" accept-charset="utf-8" name="htst_shortcode">
                    ...
                    <div id="tst_single_wrap" style="margin-top: 10px; margin-bottom: 5px;">
                        <select name="tst_single" id="tst_single" size="1">
                            ...
                            // 'get_posts' function instead of 'new WP_Query' class to become compatible with the last version of Yoast seo
                            $tst_query = get_posts($tst_args);
 
                            //Start Loop to display testimonial(s)
                            foreach ($tst_query as $tst_testimonial) {
                              $tstid= $tst_testimonial->ID;
                              $tst_title= $tst_testimonial->post_title
                              ?>
					...
					<div id="tst_random_single_tst_wrap">
						<label for="tst_random_single_tst" style="color: blue;"><?php _e('Random Single Testimonial', 'hndtst_loc'); ?></label>
						<input type="checkbox" name="tst_random_single_tst" value="yes" id="tst_random_single_tst" />
						<br /><span style="font-size: 8px; color: darkgrey; font-style: italic">Show different testemonial on each page load</span>
					 </div>
            <h1 style="cursor:pointer;" onclick="
              javascript:
              jQuery('#hndtst_tst_generator_style_section').toggle('slow')
            ">Style Settings</h1>
                  <div id="hndtst_tst_generator_style_section">
            ...
            <h1 style="cursor:pointer;" onclick="
              javascript:
              jQuery('#hndtst_tst_generator_advanced_section').toggle('slow')
            ">Advanced Options</h1>
            <span id="hndtst_tst_generator_advanced_section">
            ...
            <!-- Column 2 -->
            <div class="hndtst-dialog_Col_2-wrap">
              <div class="hndtst-dialog_Col_2">
                  ...
                  <div id="tst_widget_instance_block">
                      <!-- Save Design Dialogue Box -->
                      <div id="tst_widget_instance_save">
                          <?php _e('Save Design ', 'hndtst_loc'); ?><br />
                      ...
                      <div id="tst_widget_instance_retrieve">
                          <?php _e('Retrieve Saved Design', 'hndtst_loc'); ?><br />
                          <?php
                          global $wpdb;
                  ...
                  <div class="hndtst-dialog_ad">
                    <div style="margin-left: auto; margin-right: auto;  margin-top: 40px; margin-bottom: 20px; width: 400px; border: 2px dotted #953E0C">
                        <?php echo '<a href="http://ristrettoapps.com/handsome-testimonials-pro/" target="blank"><img src="' . TSTMT_PLUGIN_URL . '/assets/images/upgrade-hndtst-pro-side-horiz.png"/></a>'; ?>
                    </div>
                </div>
...
function hndtst_action_instance_callback() {
    ...
    if ($row) {
        ...
        //echo($hndtst_shortcode); // Don't remove this line it depends with the "start_of_shortcode = response.lastIndexOf('[');" in the admin-scripts.js file
    }
...
function hndtst_action_design_options_callback() {
  ...
  if ($hndtst_design_options_name == '') {
    ...
    $sql = "SELECT * FROM `". $table ."` WHERE `name`='".$_POST['hndtst_design_options_name']."'";
    $res = $wpdb->get_results($sql);

    // $sql = "DELETE FROM `" . $table . "` WHERE `name`='".$_POST['hndtst_design_options_name']."'";

    if(!empty($res)) {
        echo json_encode(array(
          'error'=> true,
          'message' => '<span style="color:red;">Widget Instance already created.</span>'
        ));
        wp_die();
    }
  ...
  if ($wpdb->insert_id){
      ...
      echo json_encode(array(
        'error' => false,
        'message' => '<span style="color:green;">Instance is successfully saved <br /><a href="'.get_site_url().'/wp-admin/widgets.php" target="blank"><i>Retreive in Widgets</i></a></span> ',
        'id'=>$wpdb->insert_id,
        'name' =>$hndtst_design_options_name
      ));
      ...
      echo json_encode(array(
        'error'=> true,
        'message' => '<span style="color:red;">Failed to save instance</span>'
        ));
  }
...
function delete_instance() {
    ...
    $sql = "DELETE FROM `". $table ."` WHERE `id`='".$_POST['instance_id']."'";
    $res = $wpdb->get_results($sql);
...
function delete_instance_name() {
    ...
    $sql = "DELETE FROM `". $table ."` WHERE `id`='".$_POST['instance_name']."'";
    $res = $wpdb->get_results($sql);
    echo 'Instance removed';
