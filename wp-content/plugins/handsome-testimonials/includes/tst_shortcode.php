 ...
 */
	...
	class handsometestimonials_shortcode {
	    ...
	    public function __construct()
	    {
	    		...
	    		add_shortcode( 'handsometestimonial', array( $this, 'dsply_testimonial' )); //Default Shortcode
	    		add_shortcode( 'testimonial_single', array( $this, 'dsply_testimonial' )); //Deprecated Shortcode
	    ...
	    function dsply_testimonial($atts) {
            ...
            $hndtst_settings=get_option('hndtst_settings');

            if($hndtst_settings && $hndtst_settings['custom_css']){

                //Append custom CSS from the general settings
                $tst_css.=$hndtst_settings['custom_css'];
            }

		 	//Enque Specified Template CSS Style
			wp_register_style( 'handsometestimonials_style', TSTMT_PLUGIN_URL . 'includes/css/testimonial.css' );
			...
			//Random single testimonial
			if(strtolower($tst_shortcode['single_random'])=='yes'){
				$tst_query_list = new WP_Query(array('post_type' => 'testimonial'));
				
				//get the list of ids from WP_Query
				 $tst_list_ids = wp_list_pluck( $tst_query_list->posts, 'ID' );				 

				//get random value from the list
				$tstid = $tst_list_ids[array_rand($tst_list_ids)];
			}
