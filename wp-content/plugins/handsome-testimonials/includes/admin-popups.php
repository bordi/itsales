...
function hntst_email_pointer_content() {
    ...
    $pointer_content .= '<p>' . __( 'Signup and we\\\'ll send you our <br /><b>Tried & True Testimonial Request Template</b>', 'hndtst_loc') . '</p>';
    $pointer_content .= file_get_contents( TSTMT_PLUGIN_DIR. '/assets/pointer-email-optin.txt');
        ...
        jQuery(document).ready( function($) {
            $('#wpadminbar').pointer({
                ...
                close: function() {
                    ...
                    setUserSetting( 'hndtst_email_pointer_display', '0' );
                    setUserSetting( 'hndtst_email_pointer_display_close_time', close_time );
                }
            }).pointer('open');
        });
        //]]>
        ...
        jQuery(document).ready( function($) {
            ...
            $('#wpadminbar').pointer({
                ...
                close: function() {
                    ...
                    setUserSetting( 'hndtst_survey_pointer_display', '0' );
                    setUserSetting( 'hndtst_survey_pointer_display_close_time', close_time );
                },
                show:function(){
                    ...
                    jQuery("#hntst_survey_rating [data-rating]").click(function(){
                        star_rated=jQuery(this).data("rating");
                        setUserSetting( 'hndtst_survey_pointer_display_rate', star_rated );
...
function hndtst_fb_enqueue_wp_pointer( $hook_suffix ) {
    ...
    $screen_id = $screen->id;
    $enqueue = FALSE;
    $admin_bar_pointer_on = get_user_setting( 'hndtst_email_pointer_display', 1 ); // check settings on user
    $survey_active = get_user_setting( 'hndtst_survey_pointer_display', 1 );

    $now=time();
    $email_pointer_close_time=get_user_setting("hndtst_email_pointer_display_close_time",0);

    $one_hour=60*60;


    //Screens to show Pointer On
    $pointer_screens = array(
        'testimonial',
        'edit-testimonial',
        'testimonial_page_htst_design',
        'testimonial_page_htst_getting_started'
        );

    // check if admin bar is active and default filter for wp pointer is true
    if (apply_filters( 'show_wp_pointer_admin_bar', TRUE ) && in_array($screen_id, $pointer_screens) ) {
        $enqueue = TRUE;
        if($admin_bar_pointer_on){
            //email pointer
            add_action( 'admin_print_footer_scripts', 'hntst_email_pointer_content' );

        } elseif ( (($now-$email_pointer_close_time) > ($one_hour*24)) && $survey_active){ //if after 3 days
...
function hntst_send_my_feedback() {
    ...
    wp_mail( "feedback@ristrettoapps.com", "Handsome Testimonial Feedback", $hntst_feedback_text);
    die(
    json_encode(
        array(
            'success' => true,
